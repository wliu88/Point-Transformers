...
from dataset import visualize_part_segmentation
...
def main(args):
    ...
    TEST_DATASET = PartNormalDataset(root=root, npoints=args.num_point, split='test', normal_channel=args.normal, class_choice=["Knife"])
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
            ...
            for i in range(cur_batch_size):
                ...
                # visualization
                # gt_seg_pc = visualize_part_segmentation(points[i], target[i], return_vis_pc=True)
                # pred_seg_pc = visualize_part_segmentation(points[i], cur_pred_val[i], return_vis_pc=True)
                # gt_seg_pc.apply_translation([-1, 0, 0])
                # pred_seg_pc.apply_translation([1, 0, 0])
                # trimesh.scene.Scene([gt_seg_pc, pred_seg_pc]).show()
